...
class KnowledgeBaseService:
    ...
    def __init__(self):
        self.vector_stores_dir = '../core/vector_store/docling_chroma_db'  # 向量库目录（如 data/vector_stores/）
    ...
    def query_knowledge_base_by_name(self, query_text: str, kb_name: str, top_k: int) -> List[
        ...
        try:
            # 初始化嵌入模型和向量库
            embeddings = get_bge_large_zh_embeddings()
            vector_store = create_or_load_vector_store(embeddings, self.vector_stores_dir, documents=None, batch_size=20, name=kb_name)
